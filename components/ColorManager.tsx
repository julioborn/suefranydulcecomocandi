...
export default function ColorManager({ store, initialColors }: Props) {
  ...
  async function createColor(e: React.FormEvent) {
    ...
    if (error) {
      setError(error.code === '23505' ? 'Ese color ya existe.' : 'No se pudo crear el color.')
    } else if (data) {
  ...
  async function saveEdit(id: string) {
    ...
    if (error) {
      setError(error.code === '23505' ? 'Ese color ya existe.' : 'No se pudo renombrar.')
    } else if (data) {
      setColors((prev) =>
  ...
  async function deleteColor(color: Color) {
    if (!confirm(`¿Eliminar "${color.name}"? Se quitará de los productos que lo tengan.`)) return
    const supabase = createClient()
    const { error } = await supabase.from('colors').delete().eq('id', color.id)
    if (!error) setColors((prev) => prev.filter((c) => c.id !== color.id))
  }
    ...
    <div className="card p-5 flex flex-col gap-4">
      <form onSubmit={createColor} className="flex gap-2">
        <input
          ...
          placeholder="Nuevo color (ej: Rosa)"
      ...
      {colors.length === 0 ? (
        <p className="text-sm text-[#c4a0b8] text-center py-6">Todavía no hay colores.</p>
      ) : (
              ...
              {editingId === color.id ? (
                <>
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') { e.preventDefault(); saveEdit(color.id) }
                      if (e.key === 'Escape') setEditingId(null)
                    }}
                    autoFocus
                    className={`${inputClass} py-1.5`}
                  />
                  <button onClick={() => saveEdit(color.id)} className="p-2 rounded-lg text-emerald-500 hover:bg-emerald-50">
                ...
                <>
                  ...
                  <button onClick={() => deleteColor(color)} className="p-2 rounded-lg text-[#c4a0b8] hover:text-rose-500 hover:bg-rose-50 transition-colors">
